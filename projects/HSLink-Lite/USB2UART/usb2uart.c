...
static void uart_rx_dma_flush(void)
{
    uint32_t dma_write_pos;
    dma_resource_t *rx_resource = &dma_resource_pools[UART_RX_DMA_RESOURCE_INDEX];

    dma_write_pos = UART_RX_DMA_BUFFER_SIZE - dma_get_remaining_transfer_size(rx_resource->base, rx_resource->channel);
    dma_write_pos %= UART_RX_DMA_BUFFER_SIZE;

    //printf("dma pos %u, rb_write_pos %u\n", dma_write_pos, rb_write_pos);

    if (dma_write_pos == rb_write_pos) {
        return;
    }

    if (dma_write_pos > rb_write_pos) {
        chry_ringbuffer_write(&g_uartrx, &uart_rx_buf[rb_write_pos], dma_write_pos - rb_write_pos);
    } else {
        chry_ringbuffer_write(&g_uartrx, &uart_rx_buf[rb_write_pos], UART_RX_DMA_BUFFER_SIZE - rb_write_pos);
        if (dma_write_pos > 0) {
            chry_ringbuffer_write(&g_uartrx, uart_rx_buf, dma_write_pos);
        }
    }

    rb_write_pos = dma_write_pos;
}
...
void uartx_io_init(void)
{
    ...
    if (!HSLink_Setting.jtag_20pin_compatible) {
        ...
        HPM_IOC->PAD[PIN_UART_RX].PAD_CTL = IOC_PAD_PAD_CTL_PRS_SET(2) | IOC_PAD_PAD_CTL_PE_SET(1) | IOC_PAD_PAD_CTL_PS_SET(1) | IOC_PAD_PAD_CTL_SPD_SET(3);
        HPM_IOC->PAD[PIN_UART_TX].PAD_CTL = IOC_PAD_PAD_CTL_PRS_SET(2) | IOC_PAD_PAD_CTL_PE_SET(1) | IOC_PAD_PAD_CTL_PS_SET(1) | IOC_PAD_PAD_CTL_SPD_SET(3);
    } else {
...
void chry_dap_usb2uart_uart_config_callback(struct cdc_line_coding *line_coding)
{
    ...
    config.rxidle_config.threshold = 4U;
